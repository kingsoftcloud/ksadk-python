...
- 修复会话历史分页、刷新后的工具结果与提问回放，统一思考流光、取消提示和审批交互；上下文入口支持悬停、点击固定、外部点击关闭和手动压缩进度。用量来自运行时，窗口容量可回退到模型目录；未知用量不会伪造百分比。
- 统一 Studio 页面配色、Agent 表单和部署表格；复用本地身份缓存展示当前凭证身份，新部署记录保存创建人快照，历史创建人缺失时不冒充其他用户。
- 纳入社区贡献的 Studio UI 改进（PR #66），保留其配色方向。
...
- 基础 wheel 不绑定单一 Agent 框架；框架依赖仍由 extras 选择。wheel 含页面资源，不捆绑 Python 依赖、Node/Codex/DSH 工具链、社区插件或用户凭证。
