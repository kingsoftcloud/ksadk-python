...
- Hosted TUI 会话复用：配合 `@kingsoftcloud/ksadk-web@0.2.13`，原生终端默认按当前业务会话复用 terminal session，并保留显式新建入口；Hosted UI 会随请求透传选中模型的 metadata，保证模型热切换、图片输入和 reasoning 能力判断一致，并修复连续问答、快速切会话和新建会话时的流式消息回写竞态。
...
- 本地开发可从 `@kingsoftcloud/ksadk-web@latest` 同步 Web UI static；0.6.6 发布候选固定使用 `@kingsoftcloud/ksadk-web@0.2.13`，避免 npm `latest` 变化影响发布包。
