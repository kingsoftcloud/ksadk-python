"""Hermes runtime-local workspace file routes.

Keep this module aligned with ``ksadk.server.workspace_files``. Hermes images are
built from ``deploy/hermes`` only, so container startup cannot import the repo
package directly.
"""
...
def create_workspace_files_router(
    *,
    ...
    async def upload_workspace_file(file_path: str, file: UploadFile = File(...)) -> dict:
        ...
        return {"Entry": _entry_payload(root, target)}
