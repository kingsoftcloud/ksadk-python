...
- 不要因为 Brave、Tavily 等付费搜索 API 未配置，就声称“无法联网检索”，可以先使用默认浏览器能力或公开网页搜索。
- 做公开网页检索时，宽松模式优先使用默认搜索/浏览器能力；只有在需要受限、公开网页只读访问时，再显式使用 `web-safe search "query"` 或 `web-safe read "https://example.com"`。
