...
selects the cloud Runtime image. See [Codex Managed Runtime](../framework/guides/managed-runtime.mdx).
</Callout>
...
testing an Agent, see [AgentKit Local Studio](/en/docs/framework/guides/agentkit-local-studio).
...
<Callout type="info" title="env file boundary">
  ...
  kept. See [Environment Variables Reference](../references/environment-variables.mdx).
</Callout>
...
Disabling observability clears the managed OTLP endpoints, headers, and protocols. Do not use `--env` to override platform-managed observability variables. See the [Environment Variables Reference](../references/environment-variables.mdx) for the protocol contract.
...
<Callout type="warning" title="lancedb is not exposed on the CLI">
  ...
  [Environment Variables Reference](../references/environment-variables.mdx).
</Callout>
