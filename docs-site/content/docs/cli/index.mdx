...
[Codex Managed Runtime](../framework/guides/managed-runtime.mdx)。
</Callout>
...
`studio/static` 是随 Python 包分发的 React 生产构建产物。公开仓、sdist 与 wheel 不包含
Studio 的 React / TypeScript 可编辑源码；`make build-wheel` 和 `make public-build-check`
会校验并打包经过审计的静态产物。

从启动工作区到创建、构建和测试 Agent 的完整流程见
[AgentKit Local Studio](/cn/docs/framework/guides/agentkit-local-studio)。
...
只有通过 canonical RuntimeEvent v2 store 持久化的 session 可被这个命令读取；旧式 SessionEvent 不会被静默转换成新的 canonical 事实。
...
<Callout type="info" title="env 文件边界">
  ...
  [环境变量参考](../references/environment-variables.mdx)。
</Callout>
...
关闭后平台会清理托管 OTLP endpoint、headers 和 protocol；不要用 `--env` 覆盖平台托管的可观测变量。环境变量协议见[环境变量参考](../references/environment-variables.mdx)。
