...
<Callout type="info" title="统一模型策略与 fallback">
...
[环境变量参考 - 统一模型策略与 fallback](../../references/environment-variables.mdx)。
</Callout>
...
<Callout type="warning">
  如果项目使用自定义 state 或 input 准备逻辑，把 hook 放在配置的入口模块里，Runner 才能稳定导入。
</Callout>
...
<Callout type="warning">
  不要提交 `.pypirc`、API Key、cookies、kubeconfig、私有 registry 凭证、客户数据、本地 session 数据库或上传文件。
</Callout>

## 公开文档规则

<Callout type="warning">
  公开文档不能发布真实内部 endpoint、access key、cookies、kubeconfig 路径、registry 名称、客户数据或私有支持 URL。公开文档应当：
