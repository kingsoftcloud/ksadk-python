...
[故障排查](../../references/troubleshooting.mdx)。
...
![本地运行流程](/assets/ksadk-local-run-flow.svg)
...
- 阅读 [配置项](./configuration.mdx)，理解 `.env`、YAML 和 CLI 覆盖顺序。
- 阅读 [运行时架构](../guides/runtime-architecture.mdx)，理解请求如何进入 Runner。
- 阅读 [部署到金山云](../guides/cloud-deployment.mdx)，把已验证的项目发布为托管 Agent。
- 阅读 [OpenAI 兼容 API](../../references/openai-compatible-api.mdx)，把本地 Agent 接到客户端。
- 阅读 [本地 Web UI](../guides/local-web-ui.mdx)，了解会话、上传和工作区预览。
