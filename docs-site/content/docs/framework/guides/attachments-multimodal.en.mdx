---
title: "Attachments And Multimodal Input"
---
...
<Callout type="info">
Application code should use the normalized runner payload instead of branching on which public protocol the client used. See [Consume Structured Attachment Fields](#consume-structured-attachment-fields).
</Callout>

## Recommended Practices

- Route image analysis to a vision-capable model (see [Default Multimodal Model](#default-multimodal-model)).
...
<Callout type="info" title="New in 0.6.6">
...
<Callout type="info" title="Local cache materialization">
...
```mermaid
flowchart TD
  classDef upload fill:#5368db,stroke:#3b46b8,color:#fff
  classDef uri fill:#59d3d9,stroke:#3aaeb4,color:#062b2d
  classDef cache fill:#eef6ff,stroke:#5368db,color:#1b2350
  classDef hosted fill:#fff4e6,stroke:#e0933a,color:#5a3a0a

  subgraph upload_path["Upload and normalize"]
    U["user uploads file"]:::upload --> UF["UploadFile<br/>POST /v1/UploadFile"]:::upload
    UF --> S["AttachmentStorageService.store"]:::upload
    S --> LC1["write bytes to local files/"]:::cache
    S --> KS3["mirror to KS3<br/>(backend=ks3/local)"]:::cache
    S --> R["return file_uri<br/>ksadk-upload://file_id"]:::uri
  end

  subgraph consume["Consume"]
    R -->|"current turn"| CA["current_attachments<br/>current_attachment_results"]:::uri
    R -->|"carryover context"| AR["attachment_results"]:::uri
    R --> WP["workspace preview"]:::uri
  end

  subgraph refresh["Refresh / session resume"]
    F["browser refresh / session resume"]:::hosted --> RC["runtime unified resolution of file_uri"]:::uri
    RC -->|"ksadk-upload://"| LCR["read local cache<br/>fall back to KS3 when missing"]:::cache
    RC -->|"ae-upload://"| AE["AttachmentContent<br/>KOP Action downloads bytes"]:::hosted
    AE --> MZ["materialize to local files/<br/>write .meta.json<br/>(backend=hosted)"]:::cache
    MZ --> C2["runner keeps reading real file content"]:::cache
    LCR --> C2
    LCR --> W2["workspace preview keeps reading real file content"]:::cache
    MZ --> W2
  end
```
