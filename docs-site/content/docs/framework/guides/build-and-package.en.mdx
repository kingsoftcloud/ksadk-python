...
The `ksadk-python` wheel ships `ksadk/server/static`, so users can open the
local UI without a Node toolchain.

<Callout type="info">
Editable UI source is not part of `ksadk-python`; it lives in the `ksadk-web`
repo only.
</Callout>
...
the pinned published `@kingsoftcloud/ksadk-web` version; select another published version via the `ksadk_web_version`
