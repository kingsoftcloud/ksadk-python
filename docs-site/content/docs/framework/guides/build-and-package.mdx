...
正式 PyPI 发布走 `.github/workflows/publish-pypi.yml`，由 GitHub Release `published` 事件或 `workflow_dispatch` 触发；workflow 执行 `make public-preflight`，其中会同步固定的已发布 `@kingsoftcloud/ksadk-web@0.3.10`（可通过 `ksadk_web_version` input 指定一个已发布版本），并校验 clean export 中经过审计的 Studio 静态产物，最后通过 OIDC Trusted Publishing 上传，不依赖长期 PyPI token。同步与校验会比较 npm tarball 中 `dist-ksadk`、`ksadk/server/static` 和 wheel 内静态文件的完整路径与内容哈希，并拒绝任何 Studio React / TypeScript 源码；任一不一致都会拒绝构建。
