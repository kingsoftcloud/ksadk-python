...
正式 PyPI 发布走 `.github/workflows/publish-pypi.yml`，由 GitHub Release `published` 事件或 `workflow_dispatch` 触发；workflow 先 `make sync-ksadk-web-static`（默认拉取固定的已发布 `@kingsoftcloud/ksadk-web@0.2.19`，可通过 `ksadk_web_version` input 指定一个已发布版本），再 `make public-preflight`，最后通过 OIDC Trusted Publishing 上传，不依赖长期 PyPI token。
