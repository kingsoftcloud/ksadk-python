...
仅配置通用 endpoint 时，KsADK 会派生 `/v1/traces`。Headers 使用逗号分隔，value 应按 RFC 3986 编码。需要第二路 CloudMonitor 时，另配 `CLOUD_MONITOR_OTLP_ENDPOINT` 或 `CLOUD_MONITOR_OTLP_TRACES_ENDPOINT` 及对应 headers；两个 exporter 读取同一批 Span，保持相同的 `trace_id` / `span_id`。变量全集见 [可观测与链路追踪](./observability-tracing.mdx)。
