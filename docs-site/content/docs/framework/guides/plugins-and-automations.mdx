...
Studio 与 Hosted UI 固定使用 `@kingsoftcloud/ksadk-web@0.3.5` 的版本化
...
通用 Python wheel 包含 Web 0.3.5 与编译后的 Studio 页面，不含 Python 依赖、Node/Codex/DSH 二进制、社区插件或凭证。基础安装声明共享框架依赖，额外能力按需安装 extra；完全断网安装还需准备依赖 wheelhouse 及原生工具链。既有云端 Agent 只有显式重新构建、更新部署后才变化。
