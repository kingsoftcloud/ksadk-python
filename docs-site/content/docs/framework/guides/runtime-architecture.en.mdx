...
flowchart LR
  ...
  subgraph Entry["entry"]
    CLI["agentengine run / web"]
  end
  subgraph Boot["startup"]
    Detect["framework detection"]
    Factory["create_runner"]
    Loader["runner.load_agent"]
  end
  subgraph HTTP["HTTP / UI layer"]
    Server["FastAPI app"]
    Proto["/v1/responses<br/>/v1/chat/completions<br/>/run_sse<br/>RunAgent"]
  end
  subgraph Conv["conversation runtime conversations.runtime"]
    Build["build_run_input<br/>history + attachments"]
    Ctx["PlatformInvocationContext<br/>invocation_id"]
    Status["append run_status<br/>in_progress / completed / failed"]
    Check["append run_checkpoint<br/>run_resume"]
    Retry["fallback model retry<br/>PTL compact retry"]
  end
  subgraph Store["session service"]
    Sessions["session + events"]
  end
  subgraph Runner["BaseRunner"]
    Invoke["invoke / stream"]
  end
  subgraph Cap["run capability layer (0.6.7)"]
    Resume["ResumeRun"]
    Preview["GetCheckpointResumePreview"]
    List["ListSessionCheckpoints"]
    Cancel["CancelRun (cooperative)"]
    Subscribe["SubscribeRunEvents"]
  end

  CLI -->|"detect"| Detect
  Detect -->|"create_runner"| Factory
  Factory -->|"load_agent"| Loader
  Loader -->|"server mode"| Server
  Server -->|"route"| Proto
  Proto -->|"invoke_conversation_once<br/>/ stream_responses_conversation_turn"| Build
  Build -->|"build context"| Ctx
  Ctx -->|"platform_invocation_scope"| Invoke
  Invoke -->|"output + metadata"| Status
  Status -->|"persist"| Sessions
  Check -->|"persist"| Sessions
  Retry -.->|"on recoverable err"| Invoke
  Sessions -->|"read events"| Subscribe
  Resume -->|"find checkpoint"| Sessions
  Resume -->|"resume_input"| Build
  Preview -->|"risk + receipts"| Sessions
  List -->|"list checkpoints"| Sessions
  Cancel -->|"cooperate at next<br/>chunk / tool boundary"| Invoke

  class CLI,Detect,Factory,Loader,Server,Proto,Build,Ctx,Status,Check,Retry,Invoke,Sessions,Resume,Preview,List,Cancel,Subscribe brand
```
