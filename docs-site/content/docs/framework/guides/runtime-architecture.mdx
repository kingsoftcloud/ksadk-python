...
flowchart LR
  ...
  subgraph HTTP["HTTP / UI 层"]
    Server["FastAPI app"]
    Proto["/v1/responses<br/>/v1/chat/completions<br/>/run_sse<br/>RunAgent"]
  end
  subgraph Conv["对话运行时 conversations.runtime"]
...
flowchart TD
  ...
  subgraph CLI["ksadk.cli (入口)"]
    Entry["agentengine run / web"]
  end
  subgraph Boot["启动准备"]
    Resolve["解析项目目录"]
    EnvLoad["加载 .env / agentengine.yaml"]
  end
  subgraph Detect["框架检测"]
...
flowchart TD
  ...
  subgraph Inputs["检测输入"]
    Dir["项目目录"]
  end
  subgraph Explicit["显式配置优先"]
    Cfg{"agentengine.yaml<br/>ksadk.yaml?"}
    Validate["校验 framework<br/>entry_point<br/>agent_variable"]
  end
  subgraph Convention["约定回退"]
    LGJson{"langgraph.json?"}
    GraphTarget["解析 graph target"]
    Scan["扫描包目录 + 入口文件<br/>agent.py / main.py / app.py"]
    Analyze["_analyze_code<br/>AST 启发式"]
  end
  subgraph Out["检测输出"]
    Result["DetectionResult"]
...
超时见 [会话与文件](../reference/runtime-sessions-files)。
