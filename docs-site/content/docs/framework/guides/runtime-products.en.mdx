...
```mermaid
flowchart TD
  CLI["agentengine CLI"] -->|submit artifact| Control["AgentEngine control plane"]
  Control -->|build / pull| Image["runtime image"]
  Image -->|deploy| Hermes["Hermes<br/>dashboard / API / terminal"]
  Image -->|deploy| OpenClaw["OpenClaw gateway"]
  Hermes -->|mount| Workspace["/_ksadk/workspace/v1/*"]
  OpenClaw -->|mount| Workspace

  subgraph hosted["hosted layer"]
    Control
    Image
  end
  subgraph products["runtime products"]
    Hermes
    OpenClaw
  end
  subgraph shared["shared workspace contract"]
    Workspace
  end
