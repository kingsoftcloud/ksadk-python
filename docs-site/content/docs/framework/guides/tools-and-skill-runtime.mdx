...
```mermaid
flowchart TD
  subgraph Agent["Agent 调用方"]
    Call["tool_dispatcher / 框架原生工具调用"]
  end
  subgraph Gateway["Tool Gateway 审批闸口"]
    Policy["读取 ToolPolicy<br/>risk_level + side_effects"]
    Decide{"需要审批?<br/>strict 且 risk≥medium"}
  end
  subgraph States["三态判定"]
    S1["approval_required<br/>返回 approval_request envelope"]
    S2["approval_rejected<br/>用户拒绝 / 命令策略拦截"]
    S3["execute<br/>调用真实 impl"]
  end
  subgraph Exec["执行边界"]
    Impl["工具实现函数"]
    Sandbox["sandbox backend<br/>隔离执行"]
  end
  Result["结构化结果<br/>ok / error_type / receipts"]

  Call --> Policy
  Policy --> Decide
  Decide -->|"是"| S1
  Decide -->|"否"| S3
  S1 -->|"用户确认 approved=true"| S3
  S1 -->|"用户拒绝 approved=false"| S2
  S1 -->|"命令策略 reject"| S2
  S3 --> Impl
  Impl --> Sandbox
  Sandbox --> Result
...
```mermaid
flowchart TD
  Entry["LLM 调用 tool_dispatcher<br/>action ∈ list/describe/call"]
  Route{"action 路由"}

  subgraph List["list 分支"]
    L1["_select_agentengine_tools<br/>按 include / profile 展开"]
    L2["过滤 dispatcher 自身"]
    L3["返回 tools + tool_count"]
  end

  subgraph Describe["describe 分支"]
    D1{"tool_name 合法?<br/>非 dispatcher 自调用"}
    D2["_build_descriptor_registry"]
    D3["返回单个 tool spec<br/>risk_level / approval / boundary"]
  end

  subgraph Call["call 分支"]
    C1["校验 tool_name + 规范化 arguments"]
    C2{"enabled?"}
    C3["ToolGateway.invoke<br/>真实工具 impl"]
    C4{"返回 approval_required?"}
    C5["透传 envelope + dispatched_tool_name"]
    C6["回包 ok + tool_name + result"]
  end

  Entry --> Route
  Route -->|"list"| L1 --> L2 --> L3
  Route -->|"describe"| D1
  D1 -->|"非法 / 自调用"| Err["missing_tool_name / dispatcher_self_call"]
  D1 -->|"合法"| D2 --> D3
  Route -->|"call"| C1 --> C2
...
```mermaid
flowchart TD
  Entry["LLM 调用 execute_skills<br/>workflow_prompt + skill_names"] --> GW["ToolGateway.invoke<br/>读 ToolPolicy risk=high"]
  Decide{"strict 且<br/>未带 approval?"}
  GW --> Decide
  Decide -->|"是"| Appr["返回 approval_required 信封<br/>approval_request.id / risk_level"]
  Decide -->|"否"| Impl["_execute_skills_impl"]
  Impl --> Backend{"create_skill_runtime_backend()<br/>解析 backend"}
  Backend -->|"disabled"| DisabledErr["SkillRuntimeError<br/>skill_runtime_disabled + hint"]
  Backend -->|"local_process"| Local["本地子进程 backend"]
  Backend -->|"e2b"| E2B["E2B sandbox backend"]
  Local --> Build["build_execute_skills_tool<br/>backend.run_workflow"]
  E2B --> Build
  Build --> Resolve["runtime_agent_env_from_process<br/>注入 KSADK_SKILL_SERVICE_* / space ids"]
  Resolve --> Run["backend.run_workflow<br/>workflow_prompt / skill_space_ids / skill_names / session_id / timeout"]
  Run --> Discover["runtime 内部：<br/>发现 → 下载 archive → sha256 → 解压 → loader → agent"]
  Discover --> Result["SkillRuntimeResult.to_dict<br/>+ execution_context"]
  Result --> Out["结构化结果返回 LLM"]
  Appr --> Out
...
```mermaid
flowchart LR
  Status["sandbox_status<br/>risk=low"] --> Repo["读 backend / ttl / session<br/>不执行命令"]
  Cmd["run_command<br/>risk=high"] --> GW1{"strict?"}
  Code["run_code<br/>risk=high"] --> GW2{"strict?"}
  GW1 -->|"是"| Appr1["approval_required"]
  GW1 -->|"否"| SB1["sandbox.run_command"]
  GW2 -->|"是"| Appr2["approval_required"]
  GW2 -->|"否"| SB2["写文件 -> sandbox.run_command"]
  SB1 --> Budget["ToolResultBudget<br/>截断/持久化 stdout stderr"]
  SB2 --> Budget
...
```mermaid
flowchart TD
  Out["工具原始输出<br/>stdout / stderr / text"] --> Len{"len > persist_threshold<br/>或 > max_chars?"}
  Len -->|"否"| Inline["直接内联<br/>截到 max_chars"]
  Len -->|"是"| Persist["写文件到 persist_dir"]
  Persist --> Envelope["返回 preview + persisted.path"]
  Inline --> Envelope
