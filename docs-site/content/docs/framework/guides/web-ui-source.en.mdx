...
# Pin the concrete 0.8.5 candidate version
...
| --- | --- | --- |
| `KSADK_WEB_VERSION` | `0.3.11` | published npm package version; release candidates must pin a concrete version |
...
- The `ksadk-python` version (e.g. `0.8.5`).
- The `KSADK_WEB_VERSION` / npm package version (e.g. `0.8.5` maps to `@kingsoftcloud/ksadk-web@0.3.11`).
- The controlled build command (e.g. `make build-frontend KSADK_WEB_VERSION=0.3.11`).
- The wheel / sdist audit result (`make public-build-check` / `twine check dist/*`).

<Callout type="info" title="0.8.5 candidate record example">
- `ksadk-python`: `0.8.5`
- npm package: `@kingsoftcloud/ksadk-web@0.3.11`
- frontend build: `make build-frontend KSADK_WEB_VERSION=0.3.11`
