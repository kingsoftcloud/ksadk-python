...
| 变量 | 默认值 | 说明 |
| --- | --- | --- |
| `KSADK_WEB_VERSION` | `0.3.7` | 已发布的 npm 包版本；发布候选必须固定为具体版本 |
...
- `KSADK_WEB_VERSION` / npm 包版本（如 `0.8.4` 对应 `@kingsoftcloud/ksadk-web@0.3.7`）。
- 受控构建命令（如 `make build-frontend KSADK_WEB_VERSION=0.3.7`）。
- wheel / sdist 审计结果（`make public-build-check` / `twine check dist/*`）。

<Callout type="info" title="0.8.4 候选记录示例">
- `ksadk-python`: `0.8.4`
- npm 包: `@kingsoftcloud/ksadk-web@0.3.7`
- 前端构建: `make build-frontend KSADK_WEB_VERSION=0.3.7`
