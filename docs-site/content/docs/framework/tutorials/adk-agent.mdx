...
- [框架接入](../guides/frameworks.mdx)：ADK runner、加载边界和检测顺序。
- [Agent 最佳实践](./best-practices/agent-best-practices.mdx)：ADK、记忆、知识库、MCP 和 Skill Runtime 模式。
- [环境变量](/cn/docs/references/environment-variables)：模型、记忆、知识库、MCP 和 tracing 变量。
- [OpenAI 兼容 API](/cn/docs/references/openai-compatible-api)：本地协议形态。
