...
```mermaid
flowchart LR
  Source["existing agent.py or package"] -->|import| Import["agentengine init --from-agent"]
  Import -->|generate| Wrapper["new KsADK project wrapper"]
  Wrapper -->|review| Review["review agentengine.yaml"]
  Review -->|local| Run["agentengine run . -i"]
  Run -->|debug| Web["agentengine web . --no-open"]

  subgraph import["import and generate"]
    Source
    Import
    Wrapper
  end
  subgraph verify["verify and run"]
    Review
    Run
    Web
  end
  ...
  class Source,Web brand
  class Import,Wrapper,Run accent
