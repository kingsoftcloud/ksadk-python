...
```mermaid
flowchart LR
  Source["已有 agent.py 或 package"] -->|导入| Import["agentengine init --from-agent"]
  Import -->|生成| Wrapper["新的 KsADK 项目包装"]
  Wrapper -->|核对| Review["检查 agentengine.yaml"]
  Review -->|本地| Run["agentengine run . -i"]
  Run -->|调试| Web["agentengine web . --no-open"]

  subgraph import["导入与生成"]
    Source
    Import
    Wrapper
  end
  subgraph verify["校验与运行"]
    Review
    Run
    Web
  end
