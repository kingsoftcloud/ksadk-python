...
- 阅读 [框架接入](../guides/frameworks.mdx)，了解框架适配和 runner 加载边界。
- 阅读 [会话与文件](/cn/docs/references/runtime-sessions-files)，了解 session、上传和工作区文件。
- 阅读 [构建与打包](../guides/build-and-package.mdx)，了解本地构建和公开 artifact 规则。
