...
7. Before publication, verify the external state with
   `make public-publish-check PUBLIC_PUBLISH_PHASE=pre-publish V=x.y.z`.
8. After publication, verify the external state with
   `make public-publish-check PUBLIC_PUBLISH_PHASE=post-publish V=x.y.z`.
...
1. **Sync frontend static assets**: `make sync-ksadk-web-static` pulls
   `dist-ksadk` from the pinned, published `@kingsoftcloud/ksadk-web` version and unpacks
