...
| `KSADK_LANGGRAPH_AUTO_CHECKPOINT` | 托管 LangGraph runner 是否注入受控 PostgreSQL saver；默认 `false`，启用时入口模块必须导出 `ksadk_graph_factory(*, checkpointer)` 和 `ksadk_prepare_state(payload, session_context)`，缺失时分别以 `LANGGRAPH_FACTORY_REQUIRED` 或 `PREPARE_STATE_HOOK_REQUIRED` fail-closed |
...
| `KSADK_SKILL_OUTPUT_TEXT_MAX_BYTES` | 返回给 Agent 的文本产物总字节上限，默认 `65536`，最大 `1048576` |
...
| 变量 | 用途 |
| --- | --- |
| `KSADK_WEB_VERSION` | `make sync-ksadk-web-static` 使用的已发布 `@kingsoftcloud/ksadk-web` npm 版本，当前默认 `0.3.11`；wheel 构建前必须先发布并验证新版本 |
...
| 变量 | 用途 |
| --- | --- |
| `KSADK_DSH_HOME` | 受管理 DSH Profile 的隔离目录；默认是工作区中的 `.agentkit/dsh-homes/<dsh-version>`，当前版本为 `0.1.5-rc.1`。显式设置后 Studio 不会改写该目录。 |
| `KSADK_DSH_PROFILE` | 要使用的 DSH Profile 名称，默认 Studio `web` profile。 |
...
| 变量 | 用途 |
| --- | --- |
| `KSADK_STUDIO_CHANNEL_DEFAULT` | Studio 是否在后台默认激活官方 Channel UI bundle；默认 `1`，设为 `0`、`false`、`no` 或 `off` 可关闭 |
| `AGENTENGINE_CHANNEL_URL` | Channel 服务 HTTPS 地址的启动默认值 |
| `KSYUN_ACCOUNT_ID` | Channel 账号标识的启动默认值 |
| `KSADK_CHANNEL_WORKSPACE_ID` | Connector workspace 的启动默认值；不设置时由 Studio 工作区稳定生成 |
| `KSADK_CHANNEL_API_TOKEN` | Channel API 管理 token；敏感值，只应通过受保护配置注入 |
