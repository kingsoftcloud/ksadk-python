...
```mermaid
flowchart LR
  Client["client"] -->|HTTP/SSE| API["/v1/responses 或 /v1/chat/completions"]
  ...
  Serialize -->|HTTP/SSE| Client

  subgraph ingress["入口与规范化"]
    API
    Normalize
    Session
  end
  subgraph exec["执行与序列化"]
    Runner
    Serialize
  end
  ...
  class Client brand
  class API,Runner accent
  class Normalize,Session,Serialize neutral
```
