...
```mermaid
flowchart LR
  Client["SDK / CLI / browser"] -->|公网| Public["PublicEndpoint"]
  Public -->|鉴权| Gateway["gateway auth and routing"]
  Gateway -->|转发| Runtime["runtime pod"]
  Runtime -->|加载| Agent["agent or runtime product"]

  subgraph edge["edge / 公开入口"]
    Public
    Gateway
  end
