...
| Field | Default | Constraint | Meaning |
| --- | --- | --- | --- |
| `SessionId` | — | required, one non-empty string | target session id; arrays, missing values, and blank-after-trim values fail |
| `CheckpointIds` | `[]` | string[], max 1000 | exact, case-sensitive match on `Metadata.checkpoint_id` |
| `EventTypes` | `[]` | string[], max 1000 | exact, case-sensitive match on `EventType` |
| `Offset` | `0` | `>=0` | event offset |
| `Limit` | `10` | `1..1000` | max number of events returned |
| `AfterSeqId` | — | `>=0` | only events with a greater `SeqId` |
| `BeforeSeqId` | — | `>=1` | only events with a lower `SeqId` |
...
| `SessionId` | normalized scalar session id |
| `CheckpointIds` / `EventTypes` | normalized filter arrays |
| `Total` | total after all filters and cursor conditions, before pagination |
| `Offset` | current offset |
| `Limit` | the limit value in effect |

Both arrays are trimmed, stripped of empty values, and stably deduplicated. Values within each
array are ORed; the filter fields and cursor conditions are ANDed. An empty array disables that
filter. Filtering and `Total` calculation happen before `Offset` / `Limit`; the latest window is
selected first and the returned page remains in ascending chronological order.
...
| Action | Path | Purpose |
| --- | --- | --- |
| `ListSessionCheckpoints` | `POST /agentengine/api/v1/ListSessionCheckpoints` | list checkpoints in an optional session scope for resume selection |
...
List checkpoints, optionally filtered by session, checkpoint ID, `RunId`, `Framework`, final
resume status, or Scope, with pagination.
...
| `SessionId` | `[]` | string or string[], max 1000 | target session filter; missing or empty means no filter |
| `CheckpointId` | `[]` | string or string[], max 1000 | checkpoint ID filter; missing or empty means no filter |
...
| `ResumeStatus` | `[]` | string[] | filter final `ResumeStatus`; the value domain is open |
| `ResumeTypes` | `[]` | string[] | filter final `Scope`: `invocation`, `shared`, `pod_local`, `process_local`, or `unknown` |
| `Offset` | `0` | `>=0` | pagination offset |
| `Limit` | `100` | `1..1000` | page size, capped at 1000 |
...
Both new filter arrays are trimmed, lowercased, and deduplicated. The runtime completes payload
conversion, resume audit, expiry, and latest-only correction before applying `ResumeStatus` and
`ResumeTypes`. It then calculates `ResumableTotal`, applies `OnlyResumable`, calculates `Total`, and
paginates.
