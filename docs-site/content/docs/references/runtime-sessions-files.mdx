...
相关变量见 [环境变量](./environment-variables.mdx)。
...
| Action | 路径 | 用途 |
| --- | --- | --- |
| `ListSessionCheckpoints` | `POST /agentengine/api/v1/ListSessionCheckpoints` | 按可选 session 范围列出 checkpoint，供 resume 选择 |
...
列出 checkpoint，支持按 session、checkpoint ID、`RunId`、`Framework`、最终恢复状态和 Scope 过滤。
...
| `SessionId` | `[]` | string 或 string[]，最多 1000 项 | 目标 session；缺省或空数组不过滤 |
| `CheckpointId` | `[]` | string 或 string[]，最多 1000 项 | checkpoint ID；缺省或空数组不过滤 |
...
| `ResumeStatus` | `[]` | string[] | 按最终 `ResumeStatus` 过滤；值域开放 |
| `ResumeTypes` | `[]` | string[] | 按最终 `Scope` 过滤：`invocation`、`shared`、`pod_local`、`process_local`、`unknown` |
| `Offset` | `0` | `>=0` | 分页偏移 |
| `Limit` | `100` | `1..1000` | 每页上限 1000 |
...
两个新增过滤数组会 trim、转小写并去重。runtime 先完成 payload 转换、resume audit、过期和
latest-only 修正，再应用 `ResumeStatus` / `ResumeTypes`；随后计算 `ResumableTotal`，再应用
`OnlyResumable`、计算 `Total` 和分页。
...
| 字段 | 默认 | 约束 | 含义 |
| --- | --- | --- | --- |
| `SessionId` | — | 必填、单个非空字符串 | 目标 session id；数组、缺省和 trim 后空字符串均失败 |
| `CheckpointIds` | `[]` | string[]，最多 1000 项 | 按 `Metadata.checkpoint_id` 精确过滤，大小写敏感 |
| `EventTypes` | `[]` | string[]，最多 1000 项 | 按 `EventType` 精确过滤，大小写敏感 |
| `Offset` | `0` | `>=0` | 事件偏移量 |
| `Limit` | `10` | `1..1000` | 返回条数上限 |
| `AfterSeqId` | — | `>=0` | 只保留 `SeqId` 大于该值的事件 |
| `BeforeSeqId` | — | `>=1` | 只保留 `SeqId` 小于该值的事件 |
...
| --- | --- |
| `Events` | 当前页事件列表 |
| `SessionId` | 规范化后的标量 session id |
| `CheckpointIds` / `EventTypes` | 规范化后的过滤数组 |
| `Total` | 全部过滤与游标条件生效后、分页前的事件总数 |
| `Offset` | 当前偏移量 |
| `Limit` | 实际生效的 limit |

两个数组均会 trim、去空并稳定去重；数组内部为 OR，字段之间以及与游标条件之间为 AND，
空数组表示不启用该过滤。过滤和 `Total` 计算先于 `Offset` / `Limit`；接口选择最新窗口后，
仍按时间正序返回当前页。
