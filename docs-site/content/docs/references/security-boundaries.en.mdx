...
flowchart TD
  ...
  subgraph checks["path safety checks"]
    Normalize
    RejectAbs
    RejectParent
    Resolve
  ...
  classDef danger fill:#fde2e2,stroke:#e5b4b4,color:#5a1a1a
  classDef neutral fill:#f5f7ff,stroke:#c9d2f5,color:#1a2240
  class Allow brand
  class Normalize,Resolve accent
  class Error danger
  class Raw,RejectAbs,RejectParent,Contained neutral
```
...
assets. See [Web UI Repository](../guides/web-ui-source).
