...
- 本地 EvalSet：选择工作区内文件。
- 云端 Dataset：从 Catalog 选择 Dataset 快照，版本由选项自动固定；普通用户不手工填写 version，历史版本仍可通过快照选项选择。
- Target：按业务类型选择 Local Source、Studio Build 或 A2A；内部 `TargetRef.locator` 由源码目录、Build ID 或 A2A 地址控件生成。
...
## 13. 2026-08-20 本地评测规则与验证更新

本轮只扩展 KsADK 本地评测合同和 CLI 模板，不修改 Studio UI。空 `--evaluator` 不再等价于固定评估器列表，而是按每个 Case 已提供的业务标准自动选择：

| Case 数据 | 自动评估器 |
| --- | --- |
| `reference_output` / `referenceOutput` / `expectedOutput` | 配置完整 Judge 时 `llm_judge@v1`，否则 `reference_match@v1` |
| `response.*` assertions | `response_contract@v1` |
| `runtime.*` assertions | `runtime_budget@v1` |
| `expectedTools` 或 `tool.*` assertions | `tool_trajectory@v1` |
| 没有响应业务标准 | 必填 `response_quality=UNAVAILABLE`，不把仅执行成功伪报为业务通过 |

显式 `--evaluator` 仍完全覆盖自动选择。V1 的 `tool.called` 和 `tool.notCalled` 保持原语义：前者只要求发生过调用，错误调用也计入；后者要求完全没有调用。

新增的 V2 断言为：

- `runtime.maxTotalTokens`：在 target 报告 token usage 时比较 `totalTokens`；未报告 usage 时结果为 `UNAVAILABLE`。
- `tool.succeeded`：至少一条同名标准化工具轨迹的状态为 `SUCCEEDED`。
- `tool.sequence`：成功工具调用按 `seqStart` 排序后，必须包含期望列表这一有序子序列。

用户可通过本地命令生成并预览合法的原生 EvalSet，不访问云端：

```powershell
uv run agentengine evalset init --template knowledge-qa --output-file evaluations\knowledge-qa.yaml
uv run agentengine evalset init --template structured-output --output-file evaluations\structured-output.yaml
uv run agentengine evalset init --template tool-routing --output-file evaluations\tool-routing.yaml
uv run agentengine evalset init --template service-sla --output-file evaluations\service-sla.yaml
uv run agentengine evalset preview --evalset-file evaluations\tool-routing.yaml --format json
```

四个模板分别覆盖参考答案、JSON 结构、工具路由和服务 SLA。已有文件必须显式传 `--force` 才会覆盖。

证据文件仍只能通过 `TraceRef` 读取；物理落盘路径现在使用 `runId + sessionId + invocationId` 的稳定 SHA-256 文件名，避免 Windows 深工作区超过路径长度限制。该目录布局不是对外合同。

实测结果：Local Source LangGraph 工具场景运行 `eval_868b3a9cbe9342e8bc8084d8db95ca5d` 为 `PASSED`，参考答案、延迟、`tool.succeeded`、`tool.sequence` 与 `expectedTools` 均为 PASS；本地 A2A 场景运行 `eval_32ac957d1f72491c9b04106d812a5581` 为 `PASSED`。Studio Build 的不可变 bundle/Operation/evidence 集成集为 6 passed。完整命令和报告位置见一期开发记录。

预发 Agent `ar-20260707174402-be8182f1` 必须使用 `--region pre-online` 查询。2026-08-20 实测控制面返回 `hush-eval_agent` 为 `RUNNING`、1/1 ready，并成功通过 CLI runtime 调用完成精确文本、约束文本和 JSON 响应场景。Endpoint 没有 A2A Agent Card，当前也没有 AgentEngine Runtime Target adapter；因此这些真实调用尚不能产生统一 `EvalRunReport`，更不能以本地 A2A fixture 代替。后续应在控制面 Endpoint/访问参数解析之上新增该 adapter，再用固定 EvalSet 执行自动评分。

本轮未实现也不应暗示已支持：工具参数/结果语义匹配、成本预算、模型时延与端到端时延拆分、跨 repetitions 的 P95/P99 聚合。
