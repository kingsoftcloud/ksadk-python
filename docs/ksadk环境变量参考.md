...
| `KSADK_WEB_VERSION` | Hosted Web UI static sync | 否 | `0.3.7` | 可显式设置已发布版本，如 `0.3.7` | 否 | 构建环境 / 发版负责人 | 否 | `make sync-ksadk-web-static` 使用的 `@kingsoftcloud/ksadk-web` npm 版本。wheel 构建必须固定一个已发布版本；升级此值前先发布并验证对应的 npm 包。 |
