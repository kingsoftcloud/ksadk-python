...
| ConversationSurface / Renderer / A2UI | 合同、投影、identity reducer、核心 Renderer、A2UI action bridge 已实现；Studio 与 Hosted UI 复用 `ksadk-web` 的版本化 headless conversation 模块，新增未知事件保留在 replay/audit 但不污染对话 | `ksadk-web@0.3.3` 正式发布后，以公开制品重建 Studio；可写 approval revision 和 Studio/Hosted UI/独立前端浏览器闭环仍需关闭 |
