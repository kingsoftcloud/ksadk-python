...
| `KSADK_A2A_A2A_AGENT_ID` | A2A Runtime | 条件必传 | 未设置 | 无 | 否 | 部署层 / 平台 | 否 | 已注册的 A2A Agent id；v2 完整 inbound JSON-RPC 装配需要此值。v1 discovery-only card 不依赖它，未注入时仍挂 `/.well-known/agent-card.json`。 |
| `KSADK_A2A_ACCOUNT_ID` | A2A Runtime | 条件必传 | 未设置 | 无 | 否 | 部署层 / 平台 | 否 | Runtime 归属账号 id（ar-* agent 的 account），v2 inbound 身份校验需要。 |
| `KSADK_A2A_AGENT_ID` | A2A Runtime | 条件必传 | 未设置 | 无 | 否 | 部署层 / 平台 | 否 | 托管 Agent 资源 id（`ar-*`）。v1 discovery-only card 只要此值非空即挂载，注册前可用。 |
| `KSADK_A2A_AGENT_NAME` | A2A Runtime | 否 | fallback `AGENTENGINE_MANAGED_RUNTIME_NAME` → `KSADK_A2A_AGENT_ID` | 无 | 否 | 部署层 / 平台 | 否 | AgentCard 展示名称；普通 Code runtime 无 `AGENTENGINE_MANAGED_RUNTIME_NAME` 时由部署层用 `agents.name` 注入。 |
...
| `KSADK_A2A_RUNTIME_ID` | A2A Runtime | 条件必传 | 等于 `KSADK_A2A_AGENT_ID` | 无 | 否 | 部署层 / 平台 | 否 | Runtime 实例 id；v1 与 `KSADK_A2A_AGENT_ID` 相同，v2 多 runtime 共享一个 agent 时再拆分。 |
