...
| `KSADK_WEB_VERSION` | Hosted Web UI static sync | 否 | `0.3.10` | 可显式设置已发布版本 | 否 | 构建环境 / 发版负责人 | 否 | `make sync-ksadk-web-static` 使用的 `@kingsoftcloud/ksadk-web` npm 版本。wheel 构建必须固定一个已发布版本；升级此值前先发布并验证对应的 npm 包。 |
...
| `AGENTENGINE_CHANNEL_URL` | Studio / Channel Connector | 否 | 未设置 | 无 | 否 | 开发者 / 平台 | 否 | 独立 Channel 服务的 HTTPS 基地址；Studio 会据此建立管理 API 和出站 WSS 连接。 |
| `KSADK_CHANNEL_WORKSPACE_ID` | Studio / Channel Connector | 否 | 由 Studio 工作区稳定生成 | 无 | 否 | 开发者 / Studio | 否 | 本地 Connector workspace 标识，用于隔离账号和 Agent 绑定。 |
| `KSADK_CHANNEL_API_TOKEN` | Studio / Channel API | 条件必传 | 未设置 | 无 | 是 | 开发者 / Secret | 否 | Channel 管理 API 的 Bearer token；只通过受保护配置注入，不写入 Agent spec、日志或 Git。 |
...
| `KSADK_DSH_HOME` | Studio 插件桥接 | 否 | `<workspace>/.agentkit/dsh-homes/0.1.5-rc.1` | 无 | 否 | 开发者 / Studio | 否 | 受管理 DSH Profile 的隔离目录。显式设置后 Studio 不会改写该目录；旧版 `.agentkit/dsh-home` 目录会保留，不会原地升级。 |
| `KSADK_DSH_PROFILE` | Studio 插件桥接 | 否 | `web` | 无 | 否 | 开发者 / Studio | 否 | 选择 DSH Profile 名称；Studio 默认使用 `web` profile，与 `KSADK_DSH_HOME` 一起用于发现已安装插件。 |
| `KSADK_DSH_BIN` | Studio 插件桥接 | 否 | 受管理固定工具链 | 无 | 否 | DSH 核心开发 / CI | 否 | 显式指定 DSH 命令路径，仅用于验证固定版本工具链；版本不匹配会拒绝执行。 |
| `KSADK_STUDIO_NO_SECURITY` | AgentKit Studio | 否 | `0` | 无 | 否 | 测试环境 | 否 | 仅受控自动化测试可设为 `1`；正常启动必须保留 loopback session 与 CSRF 校验。 |
| `KSADK_STUDIO_CHANNEL_DEFAULT` | AgentKit Studio | 否 | `1` | 无 | 否 | 开发者 / Studio | 否 | Studio HTTP 服务可用后是否后台默认激活官方 Channel UI bundle；设为 `0`、`false`、`no` 或 `off` 可关闭。失败不会阻塞 Studio 主页面。 |
...
| `KSADK_RESOURCE_IAM_ENDPOINT` | Studio 平台资源授权 | 否 | 根据 AgentEngine Server 地址选择公网或内网 IAM | 无 | 否 | 平台运维 | 否 | 覆盖知识库、记忆库和 Skill Space 绑定前身份校验使用的 IAM endpoint。仅使用平台运维提供的地址，不要写入项目文件。 |
