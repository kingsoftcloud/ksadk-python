...
| 属性 | 说明 |
| --- | --- |
| `gen_ai.agent.id` | 平台 Agent ID |
| `gen_ai.agent.name` | 平台 Agent 名称 |
| `gen_ai.conversation.id` | 平台会话 ID |
| `agentkit.run.id` | 单次运行 ID |
| `agentkit.response.id` | `/v1/responses` 输出 ID |
| `agentkit.runtime.framework` | `adk` / `langchain` / `langgraph` / `deepagents` / `openclaw` |
| `agentkit.runtime.type` | `python_runner` / `remote_runner` / `container_runtime` |
| `agentkit.agent.parent_name` | 父 Agent 名称 |
| `agentkit.agent.branch` | parallel branch 名称 |
...
| `gen_ai.usage.input_tokens` | Runtime/Provider 实际上报的输入 Token |
| `gen_ai.usage.output_tokens` | Runtime/Provider 实际上报的输出 Token |
| `agentkit.duration.ms` | Runtime 实际耗时或 Studio 回退耗时 |
| `agentkit.duration.source` | `runtime` / `studio`，不得省略回退来源 |

### 5.1.1 RuntimeEvent、OTLP 与 UI 读模型

三层合同必须分开，避免再出现 Runtime 私有事件直接驱动前端的情况：

1. `RuntimeEvent` 是运行时实时合同，负责流式文本、Tool、Usage 和生命周期；
2. `TraceAdapter` 把标准事件单向投影为 OTLP，OTLP JSON 是 Trace 持久化事实源；
3. `TraceView` 从 OTLP 无损派生，只服务 Trace Explorer，不得反向影响 Runtime 或会话状态。

本地 Studio Phase 1 已实现 `resourceSpans -> scopeSpans -> spans` 的 OTLP JSON、32 位
Trace ID、16 位 Span ID，以及 Agent / Model / Tool Span。原生 Runtime 可以直接提供
`stream_runtime_events()`；通用 `RunnerRuntimeAdapter` 优先消费该标准事件流，并重新绑定
平台公开的 Run/Session 标识，不再要求 Codex 把事件压平成临时字典后重新解析。

指标语义固定如下：

- Token 仅来自 `USAGE_REPORTED`；未上报为 `null`，不能用 tokenizer 或字符数估算；
- Duration 优先来自 `RUN_COMPLETED.duration_ms`；缺失时可用 Studio 单调时钟，但必须标记
  `agentkit.duration.source=studio`；
- Prompt、模型输出、命令、cwd、Secret 和 Token 值默认不进入 OTLP Attributes/Events；
- Trace Explorer 直接加载 Trace/Span，不通过 Chat 页面，不改变当前 Agent 或 Session。
...
当前本地 Studio 已有 `RuntimeEvent -> OTLP` 基础 Adapter；multi-agent 阶段仍需补充
`AgentEvent -> RuntimeEvent/OTel child span` 映射，才能把自有编排完整接入同一 Trace Plane。
