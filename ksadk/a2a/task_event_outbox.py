...
class SQLiteA2ATaskEventOutbox(A2ATaskEventOutbox):
    ...
    def _initialize_sync(self) -> None:
        ...
        try:
            ...
            connection.execute(
                ...
                ON a2a_task_event_outbox(created_at, batch_id)
                """
            )
            connection.commit()
        finally:
    ...
    async def enqueue(
        ...
        *,
        ...
        await asyncio.to_thread(
            self._execute,
            """
            INSERT OR IGNORE INTO a2a_task_event_outbox(
                batch_id, platform_task_id, events_json
            ) VALUES (?, ?, ?)
            """,
            (batch_id, platform_task_id, events_json),
        )
    ...
    async def pending(self, *, limit: int = 100) -> list[A2ATaskEventBatch]:
        ...
        return [
            A2ATaskEventBatch(
                batch_id=str(row[0]),
                platform_task_id=str(row[1]),
                events=list(json.loads(str(row[2]))),
    ...
    def _pending_sync(self, limit: int) -> list[tuple[Any, ...]]:
        ...
        try:
            return list(
                connection.execute(
                    """
                    SELECT batch_id, platform_task_id, events_json, attempt_count
