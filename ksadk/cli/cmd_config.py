...
def _apply_set_command(
    set_items: tuple, output_path: Path, env_path: Path, is_global: bool
) -> dict:
    ...
    if not updates_yaml and not updates_env and invalid_items:
        raise usage_error(
            ...
            hints=["示例: `agentengine config set region=cn-beijing-6 OPENAI_MODEL_NAME=deepseek-v4.1-flash`"],
        )
...
def run_config_wizard(config_file: str | None, set_items: tuple, is_global: bool):
    ...
    if set_items:
        print_warn(
            "`agentengine config wizard --set ...` 是兼容入口，"
            "推荐改用 `agentengine config set KEY=VALUE ...`。"
        )
    ...
    ensure_json_output_supported(
        ...
        suggestion=(
            "请改用 `agentengine config show --output json` 或 "
            "`agentengine config set KEY=VALUE`。"
        ),
    )
    ...
    new_config["name"] = _ask_or_exit(
        questionary.text("Agent 名称:", default=default_name, style=_questionary_style())
    )

    new_config["description"] = _ask_or_exit(
        questionary.text(
            "Agent 描述:",
    ...
    new_config["framework"] = _ask_or_exit(
        questionary.select(
            "选择开发框架:",
            choices=frameworks,
    ...
    new_env["OPENAI_API_KEY"] = _ask_or_exit(
        questionary.password(
            "API Key (OPENAI_API_KEY):", default=default_api_key, style=_questionary_style()
        )
    )

    new_env["OPENAI_BASE_URL"] = _ask_or_exit(
        questionary.text(
    ...
    new_env["OPENAI_MODEL_NAME"] = _ask_or_exit(
        questionary.text(
            "模型名称 (OPENAI_MODEL_NAME) [选填,默认使用金山云星流平台deepseek-v4.1-flash]:",
    ...
    should_config_ksyun = _ask_or_exit(
        questionary.confirm(
            "是否配置金山云凭证?",
    ...
    if should_config_ksyun:
        new_env["KSYUN_ACCESS_KEY"] = _ask_or_exit(
            questionary.password(
                "Access Key (AK):",
        ...
        new_env["KSYUN_SECRET_KEY"] = _ask_or_exit(
            questionary.password(
                "Secret Key (SK):",
        ...
        new_env["KSYUN_ACCOUNT_ID"] = _ask_or_exit(
            questionary.text(
                "Account ID (账户ID):",
    ...
    should_config_registry = _ask_or_exit(
        questionary.confirm(
            "是否使用 container 模式部署?",
    ...
    if should_config_registry:
        new_env["KCR_USERNAME"] = _ask_or_exit(
            questionary.text(
        ...
        new_env["KCR_PASSWORD"] = _ask_or_exit(
            questionary.password(
                "KCR 密码或 Token:",
    ...
    with open(output_path, "w", encoding="utf-8-sig") as f:
        ...
        remaining = {
            ...
            if k not in ["name", "version", "framework", "entry_point", "agent_variable", "region"]
        }
        if remaining:
            f.write("\n# Advanced Settings\n")
    ...
    # 情况2: 全局配置不存在 -> 首次运行，提示保存
        ...
        should_save_global = _ask_or_exit(
            questionary.confirm(
...
@click.option(
    "--set", "-s", "set_items", multiple=True, hidden=True, help="(兼容) 设置配置项 key=value"
)
@click.option(
    "--global",
    "is_global",
...
@click.option(
    ...
    "config_file",
    default=None,
    help="配置文件路径（默认自动复用 agentengine.yaml/ksadk.yaml）",
)
...
@cli_output_option(hidden=True)
def config_wizard(
    config_file: str | None, set_items: tuple, is_global: bool, output_mode: str | None
):
    """通过交互式向导配置项目。"""
    _ = output_mode
...
def config_set(set_items: tuple, is_global: bool, output_mode: str | None):
    ...
    示例:
      ...
      agentengine config set OPENAI_MODEL_NAME=deepseek-v4.1-flash OPENAI_BASE_URL=https://example.com/v1
