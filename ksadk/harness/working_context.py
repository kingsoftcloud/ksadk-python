...
def record_tool_failure(working: WorkingContext, *, name: str, error: str) -> WorkingContext:
    """记录一次工具失败/审批拒绝（有界，最新在前段保留最近 N 条）。"""
    entry = f"{name}: {error}"[:512]
    failures = working.recent_tool_failures + (entry,)
    return working.model_copy(update={"recent_tool_failures": failures[-_MAX_TOOL_FAILURES:]})


def record_tool_result(working: WorkingContext, *, name: str, result_text: str) -> WorkingContext:
    ...
    return working.model_copy(update={"verified_facts": merged[-_MAX_VERIFIED_FACTS:]})
