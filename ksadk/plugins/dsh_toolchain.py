...
DSH_GITHUB_TAG = "dsh-v0.1.5-rc.1"
DSH_GITHUB_COMMIT = "183f08e9c6dde7e36cd2318eaee70b0da08fb35e"
...
def _dependency_policy() -> str:
    """Keep the official synchronized DSH packages on the reviewed release.

    The published CLI uses caret dependencies even for release candidates.
    Pinning only the CLI would allow its Core/client packages to resolve to a
    later candidate. The pnpm hook changes resolution, not upstream source.
    """

    return (
        "// Generated by KsADK; the complete DSH release is an atomic dependency.\n"
        f"const version = {json.dumps(DSH_VERSION)};\n"
        "module.exports = { hooks: { readPackage(pkg) {\n"
        "  for (const field of ['dependencies', 'optionalDependencies', 'peerDependencies']) {\n"
        "    for (const name of Object.keys(pkg[field] || {})) {\n"
        "      if (name === '@deepseek-ai/dsh' || name.startsWith('@deepseek-ai/dsh-')) {\n"
        "        const requested = pkg[field][name];\n"
        "        if (!/^[~^<>=*xX0-9][0-9A-Za-z.*+~^<>=| -]*$/.test(requested)) {\n"
        "          throw new Error('DSH dependencies require a release version: ' + name);\n"
        "        }\n"
        "        if (/^\\d+\\.\\d+\\.\\d+(?:-[0-9A-Za-z.-]+)?$/.test(requested)\n"
        "            && requested !== version) {\n"
        "          throw new Error('DSH dependency version conflicts with host: ' + name);\n"
        "        }\n"
...
class DshToolchainStatus(_ToolchainModel):
    ...
    expected_version: Literal["0.1.5-rc.1"] = DSH_VERSION
    installed: bool
    usable: bool
    root: str
...
class DshToolchainManager:
    ...
    def status(self) -> DshToolchainStatus:
        ...
        if not self._receipt_is_valid():
            return self._status(
                ...
                executable=str(executable),
                actual_version=actual,
                problem="receipt_missing_or_mismatched",
            )
        return self._status(
            installed=True,
            usable=True,
    ...
    def _manifest_is_pinned(self, path: Path | None = None) -> bool:
        ...
        try:
            ...
            return False
        try:
            policy = (manifest_path.parent / ".pnpmfile.cjs").read_text(encoding="utf-8")
    ...
    def _lockfile_is_pinned(self, path: Path | None = None) -> bool:
        ...
        try:
            ...
            return False
        # Package and snapshot keys include peer suffixes. Match the package's
        # own version before the suffix rather than a dependency mentioned in it.
        packages = re.findall(
            r"^  ['\"]?(@deepseek-ai/dsh(?:-[a-z0-9-]+)?)@([^('\":\s]+)",
            value,
            re.MULTILINE,
        )
        return (DSH_PACKAGE, DSH_VERSION) in packages and all(
            version == DSH_VERSION for _name, version in packages
        )
    ...
    def _core_package_graph_is_complete(self, root: Path | None = None) -> bool:
        ...
        for package in DSH_CORE_PACKAGES:
            ...
            if (
                ...
                or installed.get("version") != DSH_VERSION
            ):
                return False
        return True

    def _receipt_is_valid(self) -> bool:
        try:
            receipt = json.loads((self._root / "toolchain.json").read_text(encoding="utf-8"))
            return receipt == self._receipt(self._root, DSH_VERSION)
        except (OSError, UnicodeError, json.JSONDecodeError):
            return False
    ...
    def _write_install_manifest(self, root: Path) -> None:
        ...
        (root / ".pnpmfile.cjs").write_text(_dependency_policy(), encoding="utf-8")

    def _write_receipt(self, root: Path, actual: str) -> None:
        self._write_json(root / "toolchain.json", self._receipt(root, actual))

    @staticmethod
    def _receipt(root: Path, actual: str) -> dict[str, Any]:
        return {
            "schemaVersion": 2,
            "package": DSH_PACKAGE,
            ...
            "dependencyPolicyDigest": "sha256:" + hashlib.sha256(
                (root / ".pnpmfile.cjs").read_bytes()
            ).hexdigest(),
        }
