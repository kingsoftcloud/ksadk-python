...
class LangChainRunner(BaseRunner):
    ...
    def load_agent(self) -> None:
        ...
        try:
            ...
            raise AttributeError(
                f"模块 {package_name} 中未找到 {self.detection_result.agent_variable}"
            )
    ...
    async def invoke(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with tracer.start_as_current_span(trace_name) as span:
            ...
            span.set_attribute("output.value", output[:5000] if output else "")
    ...
    async def stream(self, input_data: Dict[str, Any]) -> AsyncIterator[Dict[str, Any]]:
        ...
        with tracer.start_as_current_span(trace_name) as span:
            ...
            try:
                ...
                # 捕获流式调用错误，允许 BaseRunner 回退到 invoke
                print(f"\n❌ LangChain 流式调用失败: {e} (将尝试回退到同步调用)")
                span.set_attribute("stream.error", str(e))
                # 不抛出异常，让函数正常结束，BaseRunner 会检测到 response_text 为空从而调用 invoke
