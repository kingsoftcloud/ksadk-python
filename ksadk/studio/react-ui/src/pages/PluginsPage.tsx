...
export function PluginsPage() {
  ...
  return <div className="page-container plugins-page" data-layout="document">
    ...
    <section className="plugin-marketplace block">
      ...
      </> : <p className="plugin-discovery-empty">当前没有可安装的 Codex 插件。</p> : <div className="plugin-dsh-market"><div className="plugin-dsh-market-copy"><span className="plugin-avatar" data-ecosystem="dsh"><Puzzle size={18}/></span><div><strong>从来源安装</strong><p>支持带精确版本的 npm 包或绝对本地路径；Git 地址、标签和版本范围不会执行。</p></div></div><div className="plugin-source-form"><label className="sr-only" htmlFor="dshSource">DSH 插件来源</label><input id="dshSource" value={source} onChange={event => setSource(event.target.value)} placeholder="精确版本 npm 包，如 @xmanrui/dsh-im@4.13.0"/><button className="button secondary" disabled={!accepted || !source.trim() || Boolean(busy)} onClick={() => void installDsh()}>{busy === "install" ? <LoaderCircle className="animate-spin" size={15}/> : <Box size={15}/>}安装到 Profile</button></div><label className="plugin-source-confirmation"><input type="checkbox" checked={accepted} onChange={event => setAccepted(event.target.checked)}/><span>我已知悉：DSH 包及安装脚本以当前系统用户权限运行。</span></label></div>}
    </section>
  </div>;
}
