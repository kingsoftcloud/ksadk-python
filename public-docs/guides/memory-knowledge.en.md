...
## OpenClaw Memory Backend (Optional)

!!! new "New in 0.6.7"
    The OpenClaw runtime can declare a memory backend through a manifest. An
    in-process LanceDB memory plugin has been added.

The OpenClaw runtime declares a memory backend through `MEMORY_BACKEND_MANIFEST`.
The `backend_type` can be one of:

| `backend_type` | Description | When to use |
| --- | --- | --- |
| `openclaw_default` | built-in OpenClaw memory | when no external memory service is needed |
| `mem0` | connect to an external mem0 service | when you already run mem0 and need shared memory across sessions/instances |
| `lancedb` | in-process LanceDB memory plugin | local persistent memory with no external dependencies |

Notes:

- `lancedb` maps to an in-process LanceDB memory plugin (plugin id
  `memory-lancedb`) and is **not** exposed through the
  `agentengine openclaw deploy --memory-system` CLI; that CLI choice is limited
  to `openclaw_default` / `mem0`.
- When `mem0` is selected, `MEM0_API_KEY` / `MEM0_USER_ID` / `MEM0_BASE_URL`
  are required (injected by the hosted platform).
- Rendering the `memory-lancedb` output disables `openclaw-mem0` to avoid
  loading both backends at once.
- The manifest `config` can optionally override `dbPath`, `embedding`, and
  `storageOptions`.

```yaml
# MEMORY_BACKEND_MANIFEST example (injected by the hosted platform)
backend_type: lancedb
config:
  dbPath: /data/agent-memory/lance
  embedding:
    provider: openai
...
!!! info "Public surface"
    The manifest and secrets above are injected by the hosted platform. Public
    docs do not expose internal endpoints, accounts, or tokens; examples use
    placeholders such as `example.com` / `sk-test` / `cm-appkey-placeholder`.

For the full variable list, see
[ksadk environment variables reference](../reference/environment-variables.en.md).
