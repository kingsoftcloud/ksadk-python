...
完整列表见 [环境变量](../reference/environment-variables.md)。

## OpenClaw memory backend(可选)

!!! new "0.6.7 新增"
    OpenClaw runtime 支持通过 manifest 声明记忆 backend，新增进程内 LanceDB 记忆插件。

OpenClaw runtime 通过 `MEMORY_BACKEND_MANIFEST` 声明记忆 backend，`backend_type` 可选：

| `backend_type` | 说明 | 适用场景 |
| --- | --- | --- |
| `openclaw_default` | OpenClaw 内置默认记忆 | 不需要外部记忆服务时 |
| `mem0` | 接入外部 mem0 服务 | 已有 mem0 部署，需要跨会话/跨实例共享记忆 |
| `lancedb` | 进程内 LanceDB 记忆插件 | 无外部依赖的本地持久化记忆 |

几个要点：

- `lancedb` 对应进程内 LanceDB 记忆插件（plugin id `memory-lancedb`），**不**走 `agentengine openclaw deploy --memory-system` CLI；该 CLI 的 Choice 仅 `openclaw_default` / `mem0`。
- 选择 `mem0` 时需提供 `MEM0_API_KEY` / `MEM0_USER_ID` / `MEM0_BASE_URL`（由托管平台注入）。
- 渲染产物 `memory-lancedb` 会 disable `openclaw-mem0`，避免重复加载。
- manifest 的 `config` 可选覆盖 `dbPath`、`embedding`、`storageOptions`。

```yaml
# MEMORY_BACKEND_MANIFEST 示例（由托管平台注入）
...
!!! info "公开口径"
    上述 manifest 与 secret 均由托管平台注入；公开文档不暴露内部 endpoint、account 或 token，示例值使用占位符（如 `example.com` / `sk-test` / `cm-appkey-placeholder`）。

完整变量列表见 [ksadk 环境变量参考](../reference/environment-variables.md)。
