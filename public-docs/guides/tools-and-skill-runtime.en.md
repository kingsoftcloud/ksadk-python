...
```mermaid
flowchart LR
  Req["HTTP / RunAgentAction request"] --> Ctx["PlatformInvocationContext"]
  Ctx --> Payload["runner payload"]
  Ctx --> Sub["SubscribeRunEvents"]
  Ctx --> Cancel["CancelRun"]
  Ctx --> Trace["trace / observability"]
```
...
!!! tip "Prefer context accessors for account boundary"
...
See [Agent Context](agent-context.en.md) for the full context fields and
accessors, and [OpenAI Compatible API](../reference/openai-compatible-api.en.md)
