...
```mermaid
flowchart LR
  Req["HTTP / RunAgentAction 请求"] --> Ctx["PlatformInvocationContext"]
...
更多上下文字段和读取方式见 [智能体上下文](agent-context.md)，HTTP 入口字段
见 [OpenAI 兼容 API](../reference/openai-compatible-api.md)。
