...
- [框架接入](frameworks.zh.md)：runner 加载和多框架适配。
- [智能体上下文](agent-context.zh.md)：结构化调用上下文。
- [附件与多模态输入](attachments-multimodal.zh.md)：文件输入归一化。
- [会话与文件](../reference/runtime-sessions-files.zh.md)：工具事件和文件引用如何进入本地会话。
