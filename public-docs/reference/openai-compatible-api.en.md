...
| `account_id` | KsADK extension | isolates Skill/Workspace/Sandbox/Memory by account boundary, written into the runtime PlatformInvocationContext |
...
| `invocation_id` | current run id, used for transcript grouping and event resume |

These fields are runner payload fields, not wire-protocol fields.

!!! info "model_options and thinking control"
    ...
    `max_reasoning_tokens<=0`), the runtime automatically injects `enable_thinking=false` and
    `chat_template_kwargs.enable_thinking=false` into `extra_body` (DeepSeek compatibility) and
    filters streaming reasoning output items.

    - CHAT-style `reasoning.effort` values: `low` / `medium` / `high` / `none`.
    - Responses-style values: `none` / `minimal` / `low` / `medium` / `high` / `xhigh`.

    The `response.reasoning.delta` event is not emitted when thinking is disabled (filtered by the
    runtime).

!!! new "Added in 0.6.7"

    - DeepSeek-compatible `extra_body` injection and streaming reasoning output item filtering
      when thinking is disabled.
    - `account_id` isolates Skill/Workspace/Sandbox/Memory by account boundary.
    - Runner-internal `invocation_id` for transcript grouping and event resume.
...
| `response.reasoning.delta` | reasoning delta when provided by the runner; not emitted when thinking is disabled (filtered by the runtime) |
