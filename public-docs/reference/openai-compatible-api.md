...
| `account_id` | KsADK 扩展 | 按账号边界隔离 Skill/Workspace/Sandbox/Memory,写入运行时 PlatformInvocationContext |
...
| `stream` | compatible | 返回 server-sent events |
| `user` | compatible | 调用方/user 标识 |
| `temperature` | compatible | 支持时的 provider 选项 |
| `max_tokens` | compatible | 支持时的 provider 选项 |
| `session_id` | KsADK 扩展 | 本地 session id |
| `model_metadata` | KsADK 扩展 | 模型能力提示 |
| `model_options` | KsADK 扩展 | provider 特定运行时选项 |
| `account_id` | KsADK 扩展 | 按账号边界隔离 Skill/Workspace/Sandbox/Memory,写入运行时 PlatformInvocationContext |
...
| `invocation_id` | 当前运行 ID,用于 transcript 分组与事件续订 |

这些是 runner payload 字段，不是 wire protocol 字段。

!!! info "model_options 与 thinking 控制"

    `model_options` 透传 provider 特定选项给支持的 Runner。0.6.7 起，当 thinking 被禁用
    （`reasoning.effort=none`、`thinking.type=disabled` 或 `max_reasoning_tokens<=0`）时，runtime
    会自动向 `extra_body` 注入 `enable_thinking=false` 与
    ...
    `response.reasoning.delta` 事件在 thinking 被禁用时不发出（被运行时过滤）。

!!! new "0.6.7 新增"

    - thinking 禁用时的 DeepSeek 兼容 `extra_body` 注入与流式 reasoning output item 过滤。
    - `account_id` 按账号隔离 Skill/Workspace/Sandbox/Memory。
    - Runner 内部 `invocation_id` 用于 transcript 分组与事件续订。
...
| `response.reasoning.delta` | Runner 提供时的 reasoning delta；thinking 被禁用时不发出（被运行时过滤） |
