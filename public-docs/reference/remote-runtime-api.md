...
| run | 调用或取消 Agent run |
| checkpoint-resume | 列出 checkpoint、预览 resume、resume 或 cancel 受 checkpoint 管控的 run（0.6.7 新增） |
| files | 上传文件、下载附件内容、读取 workspace 文件和导出 workspace archive |
| bootstrap | 获取 UI bootstrap metadata，含 CustomUI、RuntimeCapabilities、CheckpointResumeCapability、RunLifecycle |
...
| 字段 | 含义 |
| --- | --- |
| `CustomUI` | 自定义 UI 资源信息（custom UI path、入口等） |
| `RuntimeCapabilities` | 运行时能力集合，标注 checkpoint/resume 等是否可用 |
| `CheckpointResumeCapability` | checkpoint/resume 能力详情 |
| `RunLifecycle` | run 生命周期语义，如是否支持 resume、cancel |

## 列表与分页字段

`ListSessions`、`ListSessionEvents` 支持分页。两套字段命名均可接受：
