...
- real `.env` / `.env.local`; Code / Container / MCP build contexts only keep
  `.env.example` / `.env.sample` / `.env.template` template files (0.6.7).
...
assets. See [Web UI Repository](../guides/web-ui-source.en.md).

!!! new "0.6.7 new"
    - The editable `ksadk/server/web-ui` source, `node_modules/`, and historical
      build artifacts are excluded from the wheel, enforced by
      `tests/test_runtime_common_packaging.py` during `make public-build-check`.
    - The wheel must contain `ksadk/server/static/index.html`, written by
      `make sync-ksadk-web-static`; a missing file fails the build directly.
    - Real `.env` / `.env.local` does not enter Code / Container / MCP build
      contexts; only `.env.example` / `.env.sample` / `.env.template` are kept.
      Runtime env is passed into the deployment payload `env_vars` via
      `--env KEY=VALUE` (repeatable) and `--env-file`, never written into the
      wheel or build context.

## Image Registry Credential Boundary

Image push/pull credentials differ by registry type, so the Kingsoft Cloud
account id is not wrongly treated as an enterprise image username:

| Registry type | Domain pattern | Credential requirement |
| --- | --- | --- |
| Personal KCR | `*.kce.ksyun.com` | `KSYUN_ACCOUNT_ID` may fall back as the username |
| Enterprise KCR | `*.ksyunkcr.com` | explicit `KCR_USERNAME` / `KCR_PASSWORD` required |
| Third-party registry | other | explicit `KCR_USERNAME` / `KCR_PASSWORD` required |

!!! warning
    `KSYUN_ACCOUNT_ID` only acts as a username fallback for personal KCR
    (`*.kce.ksyun.com`); enterprise KCR and third-party registries must set
    `KCR_USERNAME` / `KCR_PASSWORD` explicitly, or authentication fails. Template
    examples use placeholders `cm-appkey-placeholder` / `sk-test`; do not commit
    real credentials.
