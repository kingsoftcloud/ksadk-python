...
def _prompt_item(content):
    ...
    return ContextItem(
        ...
        priority=0,
        estimated_tokens=get_default_token_counter().count_text(content),
        required=True,
        droppable=False,
    )
...
def _input_item(text):
    ...
    return ContextItem(
        ...
        trust_level="user",
        priority=0,
        estimated_tokens=get_default_token_counter().count_text(text),
        required=True,
        droppable=False,
    )
...
def test_end_to_end_hosted_path():
    ...
    provider.upsert(
        MemoryRecord(
            ...
            content_hash=content_hash("偏好中文回答"),
            version=1,
        ),
    ...
    request = ContextContributionRequest(
        user_input="中文", session_id="s", invocation_id="i", user_id="u1"
    )
    ...
    budget = build_budget(
        policy=ContextBudgetPolicy(), context_window_tokens=200000, reserved_output_tokens=8000
    )
    plan = ContextPlanner().plan(
        candidates, budget=budget, integration_mode="ksadk_hosted", accounting_accuracy="estimated"
    )
...
def test_end_to_end_memory_does_not_override_safety():
    ...
    for i in range(10):
        provider.upsert(
            MemoryRecord(
                ...
                source_event_ids=[],
                source_seq_range=None,
                content_hash=content_hash(f"事实{i}"),
                version=1,
            ),
            expected_version=None,
        )
    ...
    request = ContextContributionRequest(
        user_input="事实", session_id="s", invocation_id="i", user_id="u1"
    )
