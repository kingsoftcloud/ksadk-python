...
_LANGGRAPH_DETECTION = SimpleNamespace(
    type=SimpleNamespace(value="langgraph"), name="r", is_valid=True,
    entry_point=None, agent_variable="graph", raw_config={},
)
...
class _RecordingRunner(BaseRunner):
    ...
    async def invoke(self, input_data):
        self.received.append({"instructions": str(input_data.get("instructions") or ""),
...
async def test_hosted_chain_runs_memory_recall_contributor(monkeypatch, tmp_path):
    ...
    provider.upsert(MemoryRecord(
        memory_id="pref1", tenant_id="local", workspace_id="local", scope="user", scope_id="u1",
        memory_type="profile", content="用户偏好中文回答", summary="用户偏好中文回答",
        status="active", confidence=0.9, importance=0.8, valid_from="", valid_to="", expires_at="",
        source_session_id="s", source_event_ids=[], source_seq_range=None,
        content_hash=content_hash("用户偏好中文回答"), version=1), expected_version=None)
    del provider  # 关连接，hosted 链路会用 resolve_default_memory_provider 重开同一文件库

    service = InMemorySessionService()
    runner = _RecordingRunner()
    await invoke_conversation_once(
        runner=runner, agent_id="a", user_id="u1", session_id="s1",
        messages=[{"role": "user", "content": "中文"}], model="m",
        prepare_runner=lambda _r, _m: None, instructions="中文",
...
def test_persistent_memory_provider_survives_reopen(tmp_path):
    ...
    p1.upsert(MemoryRecord(
        memory_id="m1", tenant_id="t", workspace_id="w", scope="user", scope_id="u1",
        memory_type="fact", content="偏好中文", summary="偏好中文", status="active",
        confidence=0.9, importance=0.8, valid_from="", valid_to="", expires_at="",
        source_session_id="s", source_event_ids=[], source_seq_range=None,
        content_hash=content_hash("偏好中文"), version=1), expected_version=None)
    del p1
    # 重开同一文件 → 数据持久化
...
async def test_evidence_driven_mismatch_triggers_circuit(monkeypatch):
    ...
    from ksadk.context_engine.capabilities import (
        is_capability_circuit_open, reset_capability_circuit,
    )
    from ksadk.runtime.hosted_finalizer import finalize_hosted_turn, FinalizeContext
    ...
    await finalize_hosted_turn(FinalizeContext(session_id='s', invocation_id='i', user_id='u', context_plan=None, shadow_context_plan={'runtime_type': 'codex'}, usage=None, runtime_type='codex'))
...
async def test_mismatch_does_not_circuit_when_usage_present():
    ...
    await finalize_hosted_turn(FinalizeContext(session_id='s', invocation_id='i', user_id='u', context_plan=None, shadow_context_plan={'runtime_type': 'codex'}, usage={'input_tokens': 10}, runtime_type='codex'))
...
def test_assert_capability_not_circuit_open_raises_when_circuit_open():
    from ksadk.context_engine.capabilities import (
        CapabilityCircuitOpen, assert_capability_not_circuit_open,
        mark_capability_mismatch, reset_capability_circuit,
    )
