...
def _record(memory_id="m1", *, scope="user", scope_id="u1", content="偏好中文", version=1, status="active", memory_type="fact", content_hash=None):
    from ksadk.memory.policy import content_hash as _ch
    return MemoryRecord(
        memory_id=memory_id, tenant_id="t", workspace_id="w", scope=scope, scope_id=scope_id,
        memory_type=memory_type, content=content, summary=content[:50], status=status,
        confidence=0.9, importance=0.8, valid_from="", valid_to="", expires_at="",
        source_session_id="s", source_event_ids=[], source_seq_range=None,
        content_hash=content_hash or _ch(content), version=version,
    )
...
def test_provider_upsert_get_search(provider):
    ...
    res = p.search(MemorySearchRequest(query="中文", scopes=[("user", "u1")], memory_types=["fact"]))
...
def test_provider_scope_isolation(provider):
    ...
    res = provider.search(MemorySearchRequest(query="中文", scopes=[("user", "u2")], memory_types=["fact"]))
...
def test_provider_superseded_not_active(provider):
    ...
    provider.upsert(_record(memory_id="m2", content="偏好英文", status="active"), expected_version=None)
    res = provider.search(MemorySearchRequest(query="偏好", scopes=[("user", "u1")], memory_types=["fact"]))
...
def test_provider_soft_delete_marks_deleted(provider):
    ...
    res = provider.search(MemorySearchRequest(query="中文", scopes=[("user", "u1")], memory_types=["fact"]))
...
def test_provider_content_hash_dedupe(provider):
    ...
    provider.upsert(_record(memory_id="a", content="同一事实", content_hash=ch), expected_version=None)
    provider.upsert(_record(memory_id="b", content="同一事实", content_hash=ch, version=2), expected_version=None)
    res = provider.search(MemorySearchRequest(query="同一", scopes=[("user", "u1")], memory_types=["fact"]))
...
def test_policy_rejects_api_key():
    cand = MemoryCandidate(
        candidate_id="c", operation="add", memory_type="fact", scope="user", scope_id="u1",
        content="the api_key=sk-abcdefghijklmnopqrstuvwxyz is here", confidence=0.99,
        importance=0.99, source_event_ids=[],
    )
...
def test_policy_rejects_explicit_sensitive_label():
    cand = MemoryCandidate(
        candidate_id="c", operation="add", memory_type="fact", scope="user", scope_id="u1",
        content="normal", confidence=0.99, importance=0.99, source_event_ids=[],
...
def test_policy_explicit_user_request_threshold():
    cand = MemoryCandidate(
        candidate_id="c", operation="add", memory_type="profile", scope="user", scope_id="u1",
        content="用 Python 3.12", confidence=0.65, importance=0.7, source_event_ids=[],
...
def test_policy_below_threshold_pending():
    cand = MemoryCandidate(
        candidate_id="c", operation="add", memory_type="profile", scope="user", scope_id="u1",
        content="maybe", confidence=0.5, importance=0.7, source_event_ids=[],
        reason="explicit_user_request",
    )
...
def test_policy_implicit_needs_observations():
    cand = MemoryCandidate(
        candidate_id="c", operation="add", memory_type="profile", scope="user", scope_id="u1",
        content="x", confidence=0.99, importance=0.99, source_event_ids=[],
        reason="implicit",
    )
...
def test_policy_model_guess_rejected():
    cand = MemoryCandidate(
        candidate_id="c", operation="add", memory_type="fact", scope="user", scope_id="u1",
        content="guess", confidence=0.99, importance=0.99, source_event_ids=[],
        reason="model_guess",
    )
    assert MemoryPolicy().evaluate(cand).decision == "reject"
...
def test_policy_delete_without_target_rejected():
    cand = MemoryCandidate(
        candidate_id="c", operation="delete", memory_type="fact", scope="user", scope_id="u1",
        content="", confidence=0.99, importance=0.99, source_event_ids=[],
        reason="",
    )
    assert MemoryPolicy().evaluate(cand).decision == "reject"
...
def test_policy_conflict_supersede():
    ...
    cand = MemoryCandidate(
        candidate_id="c", operation="update", memory_type="fact", scope="user", scope_id="u1",
        content="新偏好", confidence=0.9, importance=0.8, source_event_ids=[],
        conflicts_with=["old"], reason="explicit_user_request",
    )
...
def test_coordinator_flush_secret_rejected(provider):
    ...
    cand = MemoryCandidate(
        candidate_id="c", operation="add", memory_type="fact", scope="user", scope_id="u1",
        content="api_key=sk-abcdefghijklmnopqrstuvwx", confidence=0.99, importance=0.99,
        source_event_ids=[],
    )
...
def test_coordinator_hard_delete_unsupported_returns_false():
    class NoHardDeleteProvider:
        ...
        def delete(self, request): raise AssertionError("should not call")
