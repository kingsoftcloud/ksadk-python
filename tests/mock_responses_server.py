    ...
    return (
        "response.completed",
        {
            ...
            "response": {
                "model": model,
                "output": [],
                "usage": {"total_tokens": total, "last_usage": {"total_tokens": total}},
            },
        },
    )
    ...
    evs.append(
        (
            "response.content_part.delta",
            {
                ...
                "part": {"type": "reasoning"},
                "delta": {"text": "Let me check the skills list."},
            },
        )
    )
    parts = [
...
SCENARIOS = {
    "thinking_list": scenario_thinking_list,
    "long": scenario_long,
    "table": scenario_table,
    "unknown_reasoning": lambda: [
        # 真正未知事件名带 delta（模拟 glm 走未识别的 reasoning 事件），应丢弃不显示
        (
            "response.glm.reasoning",
            {"type": "response.glm.reasoning", "delta": "我是思考过程不该显示"},
        ),
...
def main():
    ...
    print(
        f"mock responses server (aiohttp) on http://127.0.0.1:{port} scenario={scenario}",
        flush=True,
    )
