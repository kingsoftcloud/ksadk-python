...
@pytest.mark.asyncio
async def test_finalize_hobby_restatement_supersedes_legacy_active_fact(
    tmp_path, monkeypatch
):
    db = tmp_path / "mem.db"
    monkeypatch.setenv("KSADK_MEMORY_FLUSH_ENABLED", "true")
    ...
    await finalize_hosted_turn(context("old", "记住我的爱好是羽毛球"))
    provider = SqliteMemoryProvider(db_path=str(db))
    from ksadk.memory.coordinator import agent_user_scope_id
    from ksadk.memory.models import MemorySearchRequest

    scope_id = agent_user_scope_id(agent_id="hobby-agent", user_id="local-user")
    old_records = provider.search(
        MemorySearchRequest(
            query="爱好",
            scopes=[("user", scope_id)],
    ...
    assert [record.content for record in old_records] == ["我的爱好是羽毛球"]
    old_id = old_records[0].memory_id

    await finalize_hosted_turn(context("new", "记住我的爱好其实是乒乓球"))
    active = provider.search(
        MemorySearchRequest(
            query="爱好",
            scopes=[("user", scope_id)],
    ...
    assert [record.content for record in active] == ["我的爱好是乒乓球"]
    assert provider.get(old_id).status == "superseded"
