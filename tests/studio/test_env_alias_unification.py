...
def test_env_file_accepts_both_aliases_and_normalizes(tmp_path, monkeypatch, alias):
    ...
    env_file = _write_env(tmp_path, f"{alias}=https://models.example/v1\nOPENAI_API_KEY=k\nOPENAI_MODEL_NAME=m\n")
...
def test_unrelated_env_still_blocked(tmp_path, monkeypatch):
    ...
    for k in ("OPENAI_BASE_URL", "OPENAI_API_BASE", "OPENAI_API_KEY", "OPENAI_MODEL_NAME", "UNRELATED_SECRET"):
        monkeypatch.delenv(k, raising=False)
    ...
    monkeypatch.setattr("ksadk.cli.cmd_studio.uvicorn.run", _cap)
    result = CliRunner().invoke(studio, [str(tmp_path / "ws"), "--no-open", "--env-file", str(env_file)])
